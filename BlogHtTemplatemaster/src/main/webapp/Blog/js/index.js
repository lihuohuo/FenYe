...
layui.define(['element', 'layer', 'form'], function (exports) {
    ...
    form.on('submit(login)', function (data) {
        ...
        setTimeout(function () {
            ...
            location.href="/UserServlet?action=login&usercode="+uname+"&userpwd="+upwd;
        }, 400);
        return false;
    });
    //检测键盘按下
    ...
    function login() {
        ...
        loginHtml += '<input type="text" name="account" lay-verify="account" placeholder="请输入账号" value="1" autocomplete="off" class="layui-input">';
        loginHtml += '</div>';
        loginHtml += '</div>';
        loginHtml += '<div class="layui-form-item">';
